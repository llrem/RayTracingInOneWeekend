...
void random_spheres() {
    ...
    r.bounce_limit = 50;
    r.background = color(0.70, 0.80, 1.00);

    r.vfov = 20;
    r.lookfrom = point3(13, 2, 3);
    r.lookat = point3(0, 0, 0);
    r.vup = vec3(0, 1, 0);
...
void earth() {
    ...
    r.vfov = 20;
    r.lookfrom = point3(0, 0, 12);
    r.lookat = point3(0, 0, 0);
    r.vup = vec3(0, 1, 0);

    r.defocus_angle = 0;

    r.render(hittable_list(globe));
}
...
void simple_light() {
    ...
    r.aspect_ratio = 16.0 / 9.0;
    r.image_width = 400;
    r.samples_per_pixel = 300;
    r.bounce_limit = 10;
    r.background = color(0, 0, 0);

    r.vfov = 20;
    r.lookfrom = point3(26, 3, 6);
    r.lookat = point3(0, 2, 0);
    r.vup = vec3(0, 1, 0);
...
void cornell_box() {
    ...
    r.aspect_ratio = 1.0;
    r.image_width = 600;
    r.samples_per_pixel = 20;
    r.bounce_limit = 100;
    r.background = color(0, 0, 0);

    r.vfov = 40;
    r.lookfrom = point3(278, 278, -800);
    r.lookat = point3(278, 278, 0);
    r.vup = vec3(0, 1, 0);
...
void final_scene(int image_width, int samples_per_pixel, int max_depth) {
    ...
    auto emat = make_shared<lambertian>(
        make_shared<image_texture>("../assets/earthmap.jpg"));
    ...
    r.bounce_limit = max_depth;
    r.background = color(0, 0, 0);

    r.vfov = 40;
    r.lookfrom = point3(478, 278, -600);
    r.lookat = point3(278, 278, 0);
    r.vup = vec3(0, 1, 0);
...
int main(){
    switch (8) {
        ...
        case 9:  final_scene(800, 10, 20);    break;
        default: final_scene(400, 100, 50);   break;
    }
}